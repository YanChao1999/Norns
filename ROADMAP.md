...
- [x] **Token usage matrix** — capture OpenAI/DeepSeek usage per stage run; card detail shows stage×tokens (Cursor often `unavailable`).
- [ ] **Agent bakeoff** — same prompt on parallel arms with different tools/settings; compare tokens + outcome in card detail.
