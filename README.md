...
- **Boards / Stages** define the workflow and per-column agent configuration. Edit stages and **transition lines** in the Control Room **Machine** view. Two default lines from one stage **split** the card into parallel tracks (for example unit tests and software). The previous column agent is told how many next stages exist and plans the split (`handoff.tracks`); one card becomes multiple with per-track title/body. After parallel work, **you** add the next column (Review, Merge to main, …) and draw lines from each track into it. That is a soft join: each forked card arrives and runs independently as soon as it is ready — open a PR, merge to `main`, etc. There is no wait-for-all barrier and no automatic card merge. Enable **Auto-start idle cards** on a column so idle work there runs without clicking Run, until a human gate (or write confirm) stops it.
- **Cards** carry the work item body and current stage pointer.
- **Agent runs** are isolated; no chat memory is shared between stages. When a board/agent has a local workspace path, each run gets a sandbox under `~/.norns/sandboxes/` via `[sandbox] backend` in `config.toml`: `directory` (default copy), `docker` (copy + long-lived container using local Docker; falls back to directory if Docker is missing), or `none`. Today this is isolation by convention (tools are pointed at the copy) — it does **not** yet block a reckless `rm -rf` on absolute host paths. Stronger governance is planned: hardened Docker (no host mounts except the copy, dropped caps, read-only rootfs), then microVM; later gVisor plus a policy layer over agent runtime. `microvm` and `gvisor` backends are reserved stubs.
