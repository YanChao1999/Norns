...
- **Agent runs** are isolated; no chat memory is shared between stages. When a board/agent has a local workspace path, each run gets a sandbox under `~/.norns/sandboxes/` via `[sandbox] backend` in `config.toml`: `directory` (default copy; isolation by convention — does not block host `rm -rf`), `docker` (copy + hardened container: only the sandbox copy mounted, `--cap-drop ALL`, read-only rootfs, no-new-privileges; falls back to directory if Docker is missing), or `none`. Later: microVM; then gVisor plus a policy layer over agent runtime. `microvm` and `gvisor` backends are reserved stubs.
