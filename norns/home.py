    ...
    path.write_text(
        "\n".join(
            [
                ...
                "",
                "[sandbox]",
                '# directory = copy only (default). docker = copy + container (falls back if Docker missing).',
                '# none = shared path. Stronger governance not enforced yet (host rm -rf still possible).',
                '# Next: hardened docker (copy-only mount, dropped caps, read-only rootfs), then microvm.',
                '# Later: gvisor + policy layer over agent runtime. microvm / gvisor = reserved stubs.',
                'backend = "directory"',
